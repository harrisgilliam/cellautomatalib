...
static unsigned char dummy[8] = "\000\000\000\000\000\000\000\000";
...
void sendmesg(int f, int m)
{
  ...
  CAM_SendPipeMesg(f, (char *) rv);
}

int CAM_SendPipeCommand(int fd, char *cmd, char *odata)
{
  char cmmd[8];
  T_ENTER("CAM_SendPipeCommand");

  bcopy(cmd, cmmd, 4);
  bcopy(odata, cmmd + 4, 4);

  if (CAM_WriteBytes(fd, cmmd, 8) < 0) {
...
int CAM_GetPipeResponse(int fd, char *ecode, char *rval)
{
  char retv[8];
...
int CAM_TwoWayPipeCmd(char *who, int wr, int rd, char *cmd, char *odata,
...
int CAM_ParsePipeMesg(int inp, int outp, char *appdata)
{
  register CMDT p;
...
int CAM_SendPipeMesg(int filenum, char *retv)
{
